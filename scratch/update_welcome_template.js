...
async function updateWelcomeTemplate() {
    try {
        await setDoc(doc(db, "mail_templates", "welcome_subscription"), {
            subject: "🌿 ¡Bienvenido a la familia Zeticas, {{name}}!",
            html: htmlContent
        });
        console.log("Template 'welcome_subscription' actualizado exitosamente!");
    } catch (e) {
        console.error("Error al actualizar el template:", e);
    }
}
